...
window.onload = () => {
    ...
    if (lower.includes("roblox") || lower.includes("duriansma")) {
        inputUser.type = "text"; 
        inputUser.placeholder = "Masukkan Username / Akun";
    } else {
        inputUser.type = "number";
        inputUser.placeholder = "Masukkan ID Akun (Angka)";
    }
...
function renderNominals() {
    ...
    list.forEach(item => {
        ...
        card.innerHTML = `
            <div class="nom-qty"><b>${item.q}</b> ${gameName.includes("SMA") ? "" : config.label}</div>
...
function gasOrder() {
    ...
    if (!userid || !selectedNominal || !payment) return alert("Lengkapi data bosku!");
