...
const hargaLain = [{q: 10, p: "Rp 4.000"}, {q: 20, p: "Rp 8.000"}, {q: 30, p: "Rp 12.000"}, {q: 40, p: "Rp 16.000"}, {q: 50, p: "Rp 20.000"}, {q: 150, p: "Rp 60.000"}, {q: 200, p: "Rp 80.000"}, {q: 300, p: "Rp 120.000"}, {q: 500, p: "Rp 200.000"}, {q: 1000, p: "Rp 400.000"}];
...
window.onload = () => {
    ...
    // 2. Set Placeholder Sesuai Game
    if (inputUser) {
        if (gameName === "Jual Rank Server DurianSMA") inputUser.placeholder = "Masukkan Nama Akun Minecraft";
        else if (gameName === "Roblox") inputUser.placeholder = "Masukkan Username Roblox";
        else if (gameName.includes("Google Play") || gameName.includes("Steam")) inputUser.placeholder = "Masukkan Nomor WA / Email";
        else inputUser.placeholder = "Masukkan Player ID / UID";
    }

    // 3. Kontrol Kolom Zone ID
    if (inputZone) {
        if (config.needsZone) {
            inputZone.style.display = "block";
            inputZone.placeholder = (gameName === "Genshin Impact") ? "Server (Asia/EU/NA)" : "Zone ID";
        } else {
            inputZone.style.display = "none";
        }
    }

    // 4. Render Nominal ke Grid
    renderNominals();
};
...
function renderNominals() {
    ...
    let listPilihan;
    if (gameName === "Jual Rank Server DurianSMA") listPilihan = daftarRank;
    else if (gameName.includes("Free Fire")) listPilihan = hargaFF;
    else if (gameName === "Mobile Legend") listPilihan = hargaML;
    else if (gameName === "Roblox") listPilihan = hargaRoblox;
    else listPilihan = hargaLain;

    grid.innerHTML = ""; 
    listPilihan.forEach(item => {
        const itemLabel = (gameName === "Jual Rank Server DurianSMA") ? "" : config.label;
        const card = document.createElement('div');
        card.className = 'nominal-item';
        card.onclick = function() { pilihNom(this, item.q); };
        card.innerHTML = `
            <div class="nom-qty"><b>${item.q}</b> ${itemLabel}</div>
...
function gasOrder() {
    ...
    const noWA = "628984830064"; 

    if (!userid || !selectedNominal) {
        return alert("Mohon isi User ID dan pilih Nominal!");
    }
    if (!payment) {
        return alert("Pilih metode pembayaran dulu, Bang!");
    }

    let pesan = `*ORDER ZEBS STORE*\n\n`;
    pesan += `🎮 Game: ${gameName}\n`;
    pesan += `🆔 User ID: ${userid}${config.needsZone ? ' (' + zoneid + ')' : ''}\n`;
    pesan += `💎 Pilihan: ${selectedNominal} ${config.label}\n`;
    pesan += `💳 Bayar: ${payment.value}\n\n`;
    pesan += `_Mohon segera diproses ya!_`;

    window.open(`https://wa.me/${noWA}?text=${encodeURIComponent(pesan)}`);
}
